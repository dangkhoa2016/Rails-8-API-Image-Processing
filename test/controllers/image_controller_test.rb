...
class ImageControllerTest < ActionDispatch::IntegrationTest
  ...
  def fixture_response(*args, expect_width:, expect_height:)
    if args.length == 3
      url, file, image_format = args
    else
      file, image_format = args
      url = "https://test.local/images/#{file}"
    end
  ...
  test "get index with valid token aborts streamed responses that exceed max size" do
    ...
    stub_request(:get, url).to_return(
      status: 200,
      headers: { "Content-Type" => "image/jpeg" },
      body: "a".b + ("b" * ImageController::MAX_RESPONSE_SIZE).b
    )
