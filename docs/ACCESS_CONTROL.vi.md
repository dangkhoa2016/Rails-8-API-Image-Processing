...
- Gọi các endpoint được bảo vệ như `/image` hoặc bất kỳ endpoint nào của `UsersController` sẽ nhận `401 Unauthorized`.
...
`ImageController` không dùng `authorize_user_access`; controller này dùng trực tiếp `before_action :authorize_request`. JWT thiếu, sai, hết hạn hoặc đã thu hồi trên `GET /image` và `POST /image` sẽ bị chặn với `401 Unauthorized` trước khi bắt đầu download hay transform.
...
- **Vai trò với Image API:** `/image` khả dụng cho mọi user đã xác thực. Kiểm soát phân quyền theo role chỉ áp dụng cho các action trong `UsersController`.
- **Rate limiting:** Áp dụng cho sign_in (5 req/60s per IP, 10 req/60s per email), registration (10 req/hr per IP), password reset (5 req/hr per IP), và `GET /image` (30 req/60s per IP). Localhost được safelist tự động.
